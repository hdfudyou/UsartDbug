...
Node * InitialList(Node *pNode)
{
    ...
    pNode = (Node *)malloc(sizeof(Node));//头节点
    if (pNode == NULL) 
    {
        return NULL;
...
//10.向单链表的表头插入一个元素
Node *InsertHeadList(Node *pNode)//x是链表的值
{
    Node *pInsert;
...
// 11.向单链表末尾的前一个节点添加一个元素
Node *InsertTailList(Node *pNode)//x是链表的值
{
    Node *pMove;
    Node *pInsert;
    pInsert = (Node *)malloc(sizeof(Node));
